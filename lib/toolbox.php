...
function query($p){
    if(isset($p['accion']) and isset($p['data'])){
        $accion =   $p['accion'];
        $data =     $p['data'];
        //$access =   isset($data['access']) ? $data['access'] : false;// variable para traer datos sin importar el logueo de usuario

        /*if(substr($accion,0,3) != "NS_"){
            if(!PYRUS_SESSION::verify_sesion()){
                response(401,'no login, no autorizado',['s_id' => session_id(),'a' => $p]);
